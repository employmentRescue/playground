...
-- -----------------------------------------------------
-- Table `playground`.`member_prefer_place`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `playground`.`member_prefer_place` (
  `member_id` BIGINT NOT NULL,
  `lat` DOUBLE NOT NULL,
  `lng` DOUBLE NOT NULL,
  `address` VARCHAR(1000) NULL,
  PRIMARY KEY (`member_id`, `lat`, `lng`),
  CONSTRAINT `fk_member_prefer_place_member_id`
    FOREIGN KEY (`member_id`)
    REFERENCES `playground`.`member_sometimes` (`member_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
