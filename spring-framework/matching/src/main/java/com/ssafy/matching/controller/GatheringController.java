...
public class GatheringController {
    ...
    @ApiOperation(value = "운동 모임 검색(구현중)", notes = "필터 조건(날짜(startDate), 지역(lat, lng), 반경(distance), 최소 시작시간(minStartTime), 최대 시작시간(maxStartTime), 수준(level), 최소 게임시간(minPlayTime), 최대 게임시간(maxPlayTime), 성별(sex), 운동종류(sports), 게임종류(gameType), 정렬조건(sort - time, people, distance))에 맞는 운동 모임을 검색해 반환한다", response = List.class)
    @PostMapping
