...
public class AuthentificationGatewayFilter implements WebFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        //
        // if (!AuthorizationHeaderContains(exchange)) return
        // serverResponseCode(exchange, HttpStatus.UNAUTHORIZED);
        //
        // String token =
        // exchange.getRequest().getHeaders().get("Authorization").get(0).substring("bearer
        // ".length());
        // KakaoLoginRefreshTokenCache kakaoLoginRefreshTokenCache =
        // loginRefreshTokenCacheRepository.findById(token).orElse(null);
        // KakaoLoginAccessTokenCache kakaoLoginAccessTokenCache =
        // loginAccessTokenCacheRepository.findById(token).orElse(null);
        //
        //
        ...
        // if (kakaoLoginAccessTokenCache.getKakao_accessToken() == null ||
        // kakaoLoginRefreshTokenCache == null) kakaoLoginAccessTokenCache = null;
        // if (kakaoLoginRefreshTokenCache.getConnected_access_token() == null)
        // kakaoLoginRefreshTokenCache = null;
        //
        //
        // System.out.println(exchange.getRequest().getHeaders());
        // System.out.println(AuthorizationHeaderContains(exchange));
        //
        // System.out.println("kakaoLoginAccessTokenCache : " +
        // kakaoLoginAccessTokenCache);
        // System.out.println("kakaoLoginRefreshTokenCache : " +
        // kakaoLoginRefreshTokenCache);
        // try {
        //
        // // refresh token 인 경우
        // /// cache에서 refresh token 관련 정보를 가져온다. (if 'instance' null or
        // connected_access_token null: throw exception)
        // /// refresh token과 연결된 access token도 캐시에서 가져온다. (if 'instance' null or
        // kakao's access_token null: throw exception)
        // /// kakao's access_token이 valid인지 체크한다. (if not valid: throw exception)
        // //// 여기까지 오면, kakao's access_token을 새로 발급 --> 전달받은 정보로 객체값 초기화 -->
        // refresh_token, access_token를 UUID로 새로 생성 --> response로 전달.
        ...
        //
        // if (kakaoLoginAccessTokenCache.getKakao_accessToken() == null ||
        // kakaoLoginRefreshTokenCache == null) throw new Exception();
        //
        // if
        ...
        //
        // kakaoLoginAccessTokenCache.setKakao_accessToken((String)
        // map.get("access_token"));;
        // if (map.get("refresh_token") != null){
        // kakaoLoginAccessTokenCache.setKakao_refreshToken((String)
        ...
        // ServerHttpResponse response = exchange.getResponse();
        //
        // response.getHeaders().add("access_token",
        // kakaoLoginAccessTokenCache.getToken());
        // response.getHeaders().add("refresh_token",
        // kakaoLoginRefreshTokenCache.getToken());
        // return response.setComplete();
        // }
        //
        //
        // // access token 인 경우
        // /// cache에서 access token 관련 정보를 가져온다. (if 'instance' null or kakao's
        // access_token null: throw exception)
        // /// kakao's access_token이 valid인지 체크한다. (if not valid: throw exception)
        // if
        // (!isValid_kakaoAccessToken(kakaoLoginAccessTokenCache.getKakao_accessToken()))
        // throw new Exception();
