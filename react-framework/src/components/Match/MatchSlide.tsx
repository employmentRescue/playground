...
export default function MatchSlide(props: Iprops) {
  return (
    <div className="w-full h-55 bg-white flex justify-between items-center rounded-15">
      {props.sports == '농구' && <img className="w-40 h-40 ml-7 mr-20" src={getImgUrl('icons', 'basketball-bg-colored')}></img>}
      {props.sports == '축구' && <img className="w-40 h-40 ml-7 mr-20" src={getImgUrl('icons', 'football-bg-colored')}></img>}
      {props.sports == '배드민턴' && <img className="w-40 h-40 ml-7 mr-20" src={getImgUrl('icons', 'badminton-bg-colored')}></img>}
