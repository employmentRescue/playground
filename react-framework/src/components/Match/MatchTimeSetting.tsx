...
export function MatchFilterTime({ shutOtherWindow, clicked, startTime }: { shutOtherWindow: ()=>void, clicked: ()=>void, startTime: (string | null)[] }) {
    ...
    return (
        <div className="flex flex-row w-74 h-25 flex-grow-0 mt-7 pt-0 pl-9 pr-6 rounded-5 bg-[#303eff]" onClick={(e)=>{ e.preventDefault(); clicked(); shutOtherWindow(); }}>
            <span className="w-43 h-15 flex-grow mt-5 p-0 font-inter text-12 font-[500] line-normal tracking-normal text-left text-[#fff]">{minStartTime + " ~ " + maxStartTime}</span>
        ...
        <div className="flex flex-col absolute top-[-117px] left-0 place-content-around w-full h-screen m-0 p-0 z-20">
            ...
            <div className="justify-center pt-10 w-full h-1/4 flex-grow-0 bg-[#fff] z-20">
                ...
                <div className="justify-center h-1/2 py-11 px-13">
                    ...
                    <div className="flex mb-12 w-full place-content-between">
                        <div className="flex">
                            ...
                            <span className="w-26 h-15 flex-grow-0 mt-3 ml-2 font-inter text-[12px] font-[500] text-left text-[#bbc0ff]">시</span>
                        </div>
                        <div className="flex">
                ...
                <div className="h-1/6 justify-center mb-15 mx-13 pt-10">
                    <div className="grid place-content-center h-34 mb-4 w-full text-center bg-[#303eff] rounded-[5px] font-inter font-[15px] text-[#fff]" onClick={(e)=>{e.preventDefault(); clicked(); setFilterData("startTime", exportedStartTime)}}>설정 완료</div>
                </div>
            </div>
        </div>
    )
